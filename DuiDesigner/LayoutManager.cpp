...
void CLayoutManager::SaveControlProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if(!pControl->IsVisible() && !((static_cast<IContainerUI*>(pControl->GetInterface(_T("IContainer"))) != NULL) && (static_cast<CContainerUI*>(pControl->GetInterface(_T("Container"))) != NULL)))
	{
		...
		if((pParent != NULL) && ((static_cast<IContainerUI*>(pParent->GetInterface(_T("IContainer"))) != NULL) && (static_cast<CContainerUI*>(pParent->GetInterface(_T("Container"))) != NULL)))
		{
			...
			//		break;
			//}
			if(!pControl->IsVisibleOnlyForDesigner())
...
void CLayoutManager::SaveRichEditProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	CRichEditUI* pEditUI = static_cast<CRichEditUI*>(pControl->GetInterface(_T("RichEdit")));

	if(pEditUI->GetWinStyle() & ES_PASSWORD)
		pNode->SetAttribute("password", "true");

	if(pEditUI->GetWinStyle() & ES_READONLY)
		pNode->SetAttribute("readonly", "true");

	if (! (pEditUI->GetWinStyle() & ES_MULTILINE) )
		pNode->SetAttribute("multiline", "false");

	if (!pEditUI->IsRich())
		pNode->SetAttribute("rich", "false");

	if (!pEditUI->IsWantCtrlReturn())
		pNode->SetAttribute("wantctrlreturn", "false");

	if (!pEditUI->IsWantReturn())
		pNode->SetAttribute("wantreturn", "false");

	if (!pEditUI->IsWantTab())
		pNode->SetAttribute("wanttab", "false");

	if (!pEditUI->IsRich())
		pNode->SetAttribute("rich", "false");

	if(pEditUI->GetVerticalScrollBar())
		pNode->SetAttribute("vscrollbar","true");

	if(pEditUI->GetHorizontalScrollBar())
		pNode->SetAttribute("hscrollbar","true");

	if(pEditUI->GetWinStyle() & ES_AUTOHSCROLL)
		pNode->SetAttribute("autohscroll", "true");

	if(pEditUI->GetWinStyle() & ES_AUTOVSCROLL)
