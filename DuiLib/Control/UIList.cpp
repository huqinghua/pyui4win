...
{
	if( _tcscmp(pstrName, DUI_CTR_LISTBODY) == 0 ) return static_cast<CListBodyUI*>(this);
	return CVerticalLayoutUI::GetInterface(pstrName);
}
...
CListTemplateElementUI::CListTemplateElementUI() : 
m_iIndex(-1),
m_pOwner(NULL), 
m_bSelected(false),
m_uButtonState(0)
...
LPCTSTR CListTemplateElementUI::GetClass() const
{
	return _T("ListTemplateElementUI");
}

UINT CListTemplateElementUI::GetControlFlags() const
{
	return UIFLAG_WANTRETURN;
}

LPVOID CListTemplateElementUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, DUI_CTR_LISTITEM) == 0 ) return static_cast<IListItemUI*>(this);
	if( _tcscmp(pstrName, DUI_CTR_LISTTEMPLATEELEMENT) == 0 ) return static_cast<CListTemplateElementUI*>(this);
	return CHorizontalLayoutUI::GetInterface(pstrName);
}

IListOwnerUI* CListTemplateElementUI::GetOwner()
{
	return m_pOwner;
}

void CListTemplateElementUI::SetOwner(CControlUI* pOwner)
{
	m_pOwner = static_cast<IListOwnerUI*>(pOwner->GetInterface(_T("IListOwner")));
}

void CListTemplateElementUI::SetVisible(bool bVisible)
{
...
void CListTemplateElementUI::SetEnabled(bool bEnable)
{
	CControlUI::SetEnabled(bEnable);
	if( !IsEnabled() ) {
		m_uButtonState = 0;
...
int CListTemplateElementUI::GetIndex() const
{
	return m_iIndex;
}

void CListTemplateElementUI::SetIndex(int iIndex)
{
	m_iIndex = iIndex;
}

void CListTemplateElementUI::Invalidate()
{
	if( !IsVisible() ) return;

	if( GetParent() ) {
		CHorizontalLayoutUI* pParentContainer = static_cast<CHorizontalLayoutUI*>(GetParent()->GetInterface(DUI_CTR_HORIZONTALLAYOUT));
		if( pParentContainer ) {
			RECT rc = pParentContainer->GetPos();
			RECT rcInset = pParentContainer->GetInset();
			rc.left += rcInset.left;
...
bool CListTemplateElementUI::Activate()
{
...
bool CListTemplateElementUI::IsSelected() const
{
	return m_bSelected;
}

bool CListTemplateElementUI::Select(bool bSelect)
{
	if( !IsEnabled() ) return false;
	if( bSelect == m_bSelected ) return true;
	m_bSelected = bSelect;
...
bool CListTemplateElementUI::IsExpanded() const
{
	return false;
}

bool CListTemplateElementUI::Expand(bool /*bExpand = true*/)
{
	return false;
}

void CListTemplateElementUI::DoEvent(TEventUI& event)
{
	if( !IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND ) {
		if( m_pOwner != NULL ) m_pOwner->DoEvent(event);
...
void CListTemplateElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	if( _tcscmp(pstrName, _T("selected")) == 0 ) Select();
	else CHorizontalLayoutUI::SetAttribute(pstrName, pstrValue);
}

void CListTemplateElementUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	if( !::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem) ) return;
	DrawItemBk(hDC, m_rcItem);
...
void CListTemplateElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
	return;
}

void CListTemplateElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	ASSERT(m_pOwner);
	if( m_pOwner == NULL ) return;
	TListInfoUI* pInfo = m_pOwner->GetListInfo();
